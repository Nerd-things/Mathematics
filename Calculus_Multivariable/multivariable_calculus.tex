...
\begin{document}
	...
	\[
		\begin{aligned}
			& = \int_a^b \frac{\partial f}{\partial t} dx_\nu - \frac{da}{dt} f(x_1, \cdots, t, \cdots, a, \cdots, x_n) + \frac{db}{dt} \; f(x_1, \cdots, t, \cdots, b, \cdots, x_n)
			\\
			& \iff a : \mathbb{R} \to \mathbb{R} \wedge b : \mathbb{R} \to \mathbb{R}
		\end{aligned}
	\]
	
	Moreover, if it happens that $a$ and $b$ are just constants, since the derivative of a constants is always 0, some terms here vanishes leaving the result of:
	
	\[
		\frac{d}{dt} \int_{a}^{b} f(x_1, \cdots, t, \cdots, x_\nu, \cdots, x_n) dx_\nu
		= \int_{a}^{b} \frac{\partial f}{\partial t}(x_1, \cdots, t, \cdots, x_\nu, \cdots, x_n) dx_\nu,
		\; a, b \in \mathbb{R}
	\]
	...
	When we are differentiating a multi-variable function, we can just use partial derivatives to differentiate with respect to a specific variable. Another way to look at it is by comprehending that we are taking a direction in an axis and differentiating the function thru that way. Taking this thinking, there are many more paths to differentiate thru than just the canonical axes and there is when the directional derivative pops up.
	
	To commence, we need to know what path we want to take and for that we need to parameterize it. 
	
	Let $a, u \in \mathbb{R}^n$ and $f : \mathbb{R}^n \to \mathbb{R}$ be continuous. We define the directional derivative as:
	
	\[
		D_u f := \lim_{t \to 0} \frac{f(a + tu) - f(a)}{t}
	\]
	
	Here, the term $a + tu$ is a line parameterized in the $u$ direction taking $a$ as displacement. We will define a few more vectors as:
	...
	\[
		...
		\begin{pmatrix}
			...
			a_n
		\end{pmatrix},
		\; \mu \in \{ x \in \mathbb{N}, 1 \le x \le n \},
	\]
	
	Here, we are defining a vector $p_\mu$ whose components $\le \mu$ are their respective component in the vector $a + tu$ but the rest of components are their respective in $a$. By that, we can establish that $p_0 = a \wedge p_n = a + tu$. Therefore, we will take the nominator of the limit and rewrite it in terms of $p_\mu$.
	
	\[
		f(a + tu) - f(a) = f(p_n) - f(p_0)
	\]
	
	This will allow us to telescope it and have a pretty neat serie:
	
	\[
		f(a + tu) - f(a) = f(p_n) - f(p_0) = \sum_{\mu = 1}^n (f(p_\mu) - f(p_{\mu - 1}))
	\]
	
	By dividing both sides by $t$:
	
	\[
		\frac{f(a + tu) - f(a)}{t} = \sum_{\mu = 1}^n \frac{f(p_\mu) - f(p_{\mu - 1})}{t}
	\]
	
	We have done it because we could make appear on the left-hand part the fraction from the definition we used for the directional derivative, obtaining a new way to express it.
	
	Now, we will define another set of functions $g_\mu : \mathbb{R} \to \mathbb{R}$ such that:
	
	\[
		g_\mu(s) = f(a_1 + tu_1, a_2 + tu_2, \cdots, a_{\mu - 1} + tu_{\mu - 1}, s, a_{\mu + 1}, \cdots, a_n)
	\]
	
	They are really similar to the vectors $p_\mu$, for all the previous components to the $\mu-th$ one, we are just placing their respective component from $a + tu$, in the $\mu-th$ component we are placing a different number $s \in \mathbb{R}$ and the rest are going to be their respective component in $a$.
	
	Hence, we can state that:
	
	\[
		f(p_\mu) - f(p_{\mu - 1}) = g_\mu(a_\mu + t u_\mu) - g_{\mu}(a_\mu)
	\]
	
	Now, we can use the mean value theorem to claim that $\exists \theta_\mu(t) \in (0, 1)$ such that:
	
	\[
		g_\mu(a_\mu + t u_\mu) - g_{\mu}(a_\mu)
		= (a_\mu + t u_\mu - a_\mu) \frac{dg_\mu}{ds}(a_\mu + \theta(t)tu_\mu)
		= t u_\mu \frac{dg_\mu}{ds}(a_\mu + \theta(t)tu_\mu)
	\]
	
	By rewriting $g'_\mu$ in terms of $f$ we see that:
	
	\[
		\frac{dg_\mu}{ds}
		= \frac{\partial f}{\partial x_\mu}(a_1 + tu_1, \cdots, a_{\mu - 1} + tu_{\mu - 1}, s, a_{\mu + 1}, \cdots, a_n)
	\]
	
	Additionally, to simplify it, we can write another set of functions $q_\mu : \mathbb{R} \to \mathbb{R}^n$ such that:
	...
	\[
		...
		\begin{pmatrix}
			...
			a_n
		\end{pmatrix}
		\implies 	g_\mu(a_\mu + t u_\mu) - g_{\mu}(a_\mu) = tu_\mu \frac{\partial f}{\partial x_\mu}(q_\mu(t))
	\]
	...
	\[
		\begin{aligned}
			& f(p_\mu) - f(p_{\mu - 1}) = g_\mu(a_\mu + t u_\mu) - g_{\mu}(a_\mu) = tu_\mu \frac{\partial f}{\partial x_\mu}(q_\mu(t))
			\\
			& \implies \frac{f(a + tu) - f(a)}{t} = \sum_{\mu = 1}^n  \frac{tu_\mu \partial_{x_\mu} f(q_\mu(t))}{t} = \sum_{\mu = 1}^n  u_\mu \frac{\partial f}{\partial x_\mu}(q_\mu(t))
		\end{aligned}
	\]
	
	Applying this result to the full directional derivative's definition:
	
	\[
		D_u f = \lim_{t \to 0} \frac{f(a + tu) - f(a)}{t} = \lim_{t \to 0} \sum_{\mu = 1}^n  u_\mu \frac{\partial f}{\partial x_\mu}(q_\mu(t))
	\]
	
	Here, the only term we have relying on t is $q_\mu$, but if we peek its definition we will see that when $t \to 0$, $q_\mu(t) \to a$.
	
	\[
		= \sum_{\mu = 1}^n  u_\mu \frac{\partial f}{\partial x_\mu}(a)
	\]
	
	We got the summation over components of a vector and partial derivatives, but we can use here the nabla operator to write it using the inner product.
	
	\[
		= \langle \nabla f(a), u \rangle = \nabla f(a) \cdot u
	\]
	
	That is why we can use also the notation $\nabla_u f$ to refer to the directional derivative.
	...
	Let us take a scalar field $f : \mathbb{R}^n \to \mathbb{R}$ and a constant $\lambda \in \mathbb{R}$. We parameterize the level curve of $f$ under the function $\vec{r} : \mathbb{R} \to \mathbb{R}^n$. Thus, we have the equation:
	...
	\[
		\frac{d}{dt} f(\vec{r}(t)) = \frac{d}{dt} \lambda
	\]
	
	We see that in the left-hand part we have to use the chain rule and on the right-hand part we just have 0:
	
	\[
		\sum_{\mu = 1}^n \frac{\partial f}{\partial x_\mu} \frac{dx_\mu}{dt} = 0
	\]
	
	Here we have exactly the inner product:
	
	\[
		\langle \nabla f, \vec{r}' \rangle = 0
	\]
	
	Therefore, we just found out that the operation $\operatorname{grad}(f) = \nabla f$ is a vector perpendicular to the parameterized path of an arbitrary level curve of $f$, which means that the gradient is pointing at any point to either a relative maximum or a relative minimum. By the nature of derivatives, we know that they are positive when the next value is greater that then current one, which means we can claim that they are actually pointing to a relative maximum.
	...
	Let us attempt to derive the following function:
	
	\[
		\frac{d}{dt_\mu}f(x_1(t_1, \cdots, t_\mu, \cdots, t_n), x_2(t_1, \cdots, t_\mu, \cdots, t_n), \cdots, x_m(t_1, \cdots, t_\mu, \cdots, t_n))
	\]
	
	We will use vector notation defining $\vec{v} : \mathbb{R}^n \to \mathbb{R}^m$ like:
	
	\[
		\frac{d}{dt_\mu}f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))
	\]
	
	Now, we will just use the standard derivative's definition.
	
	\[
		\frac{d}{dt_\mu}f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))
		= \lim_{h \to 0} \frac{f(\vec{v}(t_1, \cdots, t_\mu + h, \cdots, t_n)) - f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))}{h}
	\]
	
	Before going on with this, we will work with the derivative of $\vec{v}$ with respect to the variable $t_\mu$:
	
	\[
		\frac{\partial \vec{v}}{\partial t_\mu}
		= \lim_{h \to 0} \frac{\vec{v}(t_1, \cdots, t_\mu + h, \cdots, t_n) - \vec{v}(t_1, \cdots, t_\mu, \cdots, t_n)}{h}
	\]
	
	Next, we will add an arbitrary function $E(h)$ whose definition is unknown but we must establish that when $h \to 0$, $E(h) \to 0$ too.
	
	\[
		\frac{\partial \vec{v}}{\partial t_\mu}
		\approx \frac{\vec{v}(t_1, \cdots, t_\mu + h, \cdots, t_n) - \vec{v}(t_1, \cdots, t_\mu, \cdots, t_n)}{h} + E(h)
	\]
	
	We can solve this equation for the first term in the nominator by subtracting both sides by $E(h)$, multiplying by $h$ and adding the second term of the nominator.
	
	\[
		\vec{v}(t_1, \cdots, t_\mu + h, \cdots, t_n)
		\approx \vec{v}(t_1, \cdots, t_\mu, \cdots, t_n) + h \frac{\partial \vec{v}}{\partial t_\mu} - hE(h)
	\]
	
	We can simplify a little the expression by using another function $o(h)$, which is a function such that:
	
	\[
		h \to 0, \quad \frac{|o(h)|}{h} \to 0
	\]
	
	$hE(h)$ satisfies that condition, thus we can replace it.
	
	\[
		\vec{v}(t_1, \cdots, t_\mu + h, \cdots, t_n)
		\approx \vec{v}(t_1, \cdots, t_\mu, \cdots, t_n) + h \frac{\partial \vec{v}}{\partial t_\mu} - o(h)
	\]
	
	We just found another way to write the variables of the function $f$ in the first term of the derivative. Thus:
	
	
	\[
		\frac{d}{dt_\mu}f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))
		= \lim_{h \to 0} \frac{f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n) + h \partial_{t_\mu} \vec{v} - o(h)) - f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))}{h}
	\]
	
	By reading the result, it is quite noticeable that the result is in fact a directional derivative.
	
	\[
		\frac{d}{dt_\mu}f(\vec{v}(t_1, \cdots, t_\mu, \cdots, t_n))
		= \langle \nabla f, \partial_{t_\mu} \vec{v} \rangle
		= \sum_{\nu = 1}^n \frac{\partial f}{\partial x_\nu} \frac{\partial x_\nu}{\partial t_\mu}
	\]
	
	Therefore, a more general way to compute a derivative for a function with $m$ variable whose variables could also be functions of $n$ variables which rely on more values and taking one of them as the variable for the differentiation is:
	...
	But, in case the variables of the function only relies on a single variable, we can use the typical Leibniz's notation for single-variable functions's derivatives.
	...
	And here, if we multiply both side by $dt$, we have a more general definition for the differential of a function:
